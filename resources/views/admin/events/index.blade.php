                ...
                <div class="card card-table">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover table-center mb-0" id="events-table">
                                <thead>
                                    <tr>
                                        <th>S.No</th>
                                        <th>Title</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="event-row">
                                        ...
                                        <td class="category">{{ $event->category ? $event->category->name : 'N/A' }}</td>
                                        <td class="event-date">{{ $event->event_date ? $event->event_date->format('M d, Y') : 'N/A' }}</td>
                                        <td class="venue">{{ $event->venue ?? 'N/A' }}</td>
                                        <td class="status">{{ ucfirst($event->status) }}</td>
                                        <td class="is-featured">{{ $event->is_featured ? 'Yes' : 'No' }}</td>
                                        <td class="text-end">
                                            <div class="actions">
                                                <a href="{{ route('events.show', $event) }}" class="btn btn-sm btn-primary mr-2">View</a>
                                                <a href="{{ route('events.edit', $event) }}" class="btn btn-sm btn-info mr-2">Edit</a>
                                                <form action="{{ route('events.destroy', $event) }}" method="POST" class="delete-form" style="display: inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire({
                                    title: 'Error!',
                                    text: data.error || 'Failed to delete the event.',
                                    icon: 'error'
                                });
                            }
                        })
